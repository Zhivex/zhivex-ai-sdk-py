...
class AgentStreamResult(OwnedStream[AgentRunResult[AgentOutputT]]):
    def __init__(self, runner: asyncio.Task[AgentRunResult[AgentOutputT]], broadcast: Broadcast[AgentEvent]) -> None:
        self._runner = runner
...
def stream_agent(
    *,
    ...
    resolved_runtime = runtime or AgentRuntime(registry=registry, observer=observer)
    broadcast = Broadcast[AgentEvent](max_events=stream_buffer_size)
