...
- Stable gateway ordering preserves the caller's primary target first. Configuring the Beta `ModelCatalog` changes only fallback scoring and capability evidence: cataloged targets use typed metadata, while uncataloged targets retain the legacy name heuristic. Removing that compatibility path or changing primary-first semantics requires the normal stable deprecation workflow.
