...
- Stable gateway ordering preserves the caller's primary target first. Configuring the Stable `ModelCatalog` changes only fallback scoring and capability evidence: cataloged targets use typed metadata, while uncataloged targets retain the legacy name heuristic. Removing that compatibility path or changing primary-first semantics requires the normal stable deprecation workflow.
...
Beta APIs are intended for early adoption with documented change management. The packaged-skill layer, native subagent tools, named DBOS/Temporal/Prefect/Restate workflow-adapter factories, evaluation helpers/experiments, A2A and AG-UI adapters, Responses-compatible hosting, the general CLI/local playground, trace artifacts, redaction policies, budget guards, and UI approval chunks follow this beta policy.

The current beta-only areas are narrower than the full agent story. Foundation model middleware helpers and the maintained `default_model_catalog` snapshot remain beta. The agent runtime—including typed context/output contracts, dynamic instructions, lifecycle hooks, run middleware, and the observer protocol—session helpers, agent skills, MCP helpers, MCP-backed registries, Postgres-backed agent stores, run-state serialization, cancellation, replay, run-snapshot helpers, and durable local-tool approval resume are now part of the documented stable surface and follow the stable-surface rules above.

Model-catalog updates may add entries, change recommendations, capabilities, lifecycle, sources, regions, or dated prices in a minor release when upstream providers change. They must not repurpose an API ID as an alias for a different version or model. Retired IDs remain explicit lifecycle records when they are operationally relevant, and pricing with an expired effective window resolves as unknown. Applications needing a fixed production policy should construct and pin their own reviewed `ModelCatalog`.

The `0.22.0` `create_file_generate_cache(...)` hardening is a compatible Beta correction: the serialized JSON shape and factory signature remain unchanged, while writes move from direct truncation to synced temporary-file publication with `os.replace(...)`. Invalid on-disk entries now fail deterministically with the existing top-level `ValidationError`; applications that previously relied on raw `JSONDecodeError`, `UnicodeDecodeError`, or `TypeError` should catch `ValidationError` instead.
