...
Workflow lease managers and `cancel_workflow(...)` remain Beta. Lease tokens are opaque ownership credentials and are never serialized into checkpoints; monotonic fencing tokens are correlation and stale-writer controls, not secrets. Checkpoint and adapter schema versions are unchanged because the new node suspension field is additive and old payloads default it to `None`. `0.16.0` still does not provide an automatic workflow migration engine.
