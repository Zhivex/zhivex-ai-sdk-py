...
Open `http://127.0.0.1:8000`. The playground uses the same `/v1/responses`
streaming endpoint as the host. The command rejects non-loopback `--host`
values; use `zhivex serve` behind application-owned authentication and normal
