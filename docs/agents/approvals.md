...
After a direct handoff, pass the original root agent to `resume_agent_run(...)`, including when reconstructing agents after a process restart. The runtime locates the suspended delegate through the root's subagents or the registry, keeps its tools and instructions, and applies the root's output type, mode, name, and description to the continuation. A root without `output_type` continues to return string output even if the delegate declares a schema. Invalid typed output fails the continuation and reconciles the claimed parent run as failed.
