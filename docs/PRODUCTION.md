...
Use `resume_agent(...)` for session/checkpoint recovery. Use `resume_agent_run(...)` to continue a suspended run after a pending approval is approved or denied. For Stable durable workflow graphs, reconstruct the exact definition with the same persistent store, then use `resume_workflow(...)` for one suspended run or `fork_workflow(...)` for an auditable new lineage from a selected checkpoint. Migrate a non-terminal schema-v1 run explicitly with `migrate_workflow_run_checkpoint(...)` while workers are stopped. Re-entering a still-running idempotent graph fails closed; set `recover_running=True` only after lease/operator reconciliation proves the previous worker is gone.
