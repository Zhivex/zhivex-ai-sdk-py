...
`GatewayConfig(on_attempt=...)` emits exactly one terminal payload per executed retry or skipped target. Every payload has `phase="finished"`, `terminal=True`, a locally deterministic `attemptId="{targetRank}:{retry}"`, provider, model, success status, provider latency, retryability, retry number, target rank, typed `errorType`, sanitized error text, and a machine-readable `reason`. Combine `attemptId` with the application request ID; it is not globally unique by itself.

Executed attempts report at least one millisecond and exclude observer execution time; targets skipped before provider invocation report zero. Success uses `ok=True`. Timeout, refusal, transport/provider errors, and policy skips use `ok=False`; refusals remain failed attempts even when the gateway returns the refusal response because fallback is disabled. Policy skips/refusals use `missing_adapter`, `model_unavailable`, `unsupported_api_surface`, `vision_unsupported`, `capability_mismatch`, `cost_unknown`, `cost_exceeds_budget`, or `provider_refusal`; ordinary executions and transport/provider failures use `reason=None`. Use `errorType` and `reason` for metrics and the redacted error text only for human diagnosis.

Both synchronous and asynchronous observers are supported. Observer exceptions are non-authoritative and do not change the provider result or trigger retries. Wrap the observer in application-owned delivery/error monitoring if telemetry loss must be alerted. The SDK does not emit a gateway start event; emit one at the application boundary when required.
