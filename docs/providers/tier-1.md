...
For exact-artifact certification, `ZHIVEX_SMOKE_PORTABLE_CERTIFICATION=1` executes streaming and structured output for the selected portable provider. vLLM additionally exercises application-supplied portable retrieval. The Qwen, Kimi, and DeepSeek policies record retrieval as `unsupported`; a missing or inapplicable operation is never synthesized as `passed`. `ZHIVEX_SMOKE_META_CERTIFICATION=1` covers Meta Standard streaming, structured output, and retrieval separately from the Contributor surface.
...
| Vertex | `VERTEX_API_KEY` for Express, or token/ADC plus project for standard Cloud; `ZHIVEX_SMOKE_VERTEX_MODEL` (see [Vertex guide](./vertex.md)) |
...
- Gemini and Vertex use `gemini-3.8-flash` as the latest text catalog reference; `gemini-3.6-flash` and `gemini-3.5-flash-lite` retain exact offline-contract evidence and reject unsupported custom sampling and assistant prefill before dispatch. Google media, Batch, Interactions, Deep Research, Live Translate, and Veo-style workflows remain native clients rather than the stable portable contract. Gemini Developer API additionally tracks GA Interactions-only `gemini-omni-1.1-flash`, Transcribe, and `gemini-3.1-flash-lite-image`; Omni is not claimed for Vertex. Retired previews and Imagen 4 are not recommendations.
...
- Meta Model API is tier-1 only for `create_meta()` with Standard `muse-spark-1.2` portable text, streaming, structured output, callable tools, the agent tool loop, and application-supplied retrieval through `PortableRetrievalConfig`. Portable retrieval injects bounded `PortableDocument` text through Chat Completions; it does not call Meta Files, hosted search, or raw Responses. Its tool choice remains `auto` only. Contributor models, `meta_hosted_tool()`, `meta_web_search_tool()`, `meta_tool_search_tool()`, Files, raw Responses/continuation, hosted tools, and multimodal/native extras remain Beta. Embeddings, speech output, transcription, grounding, Realtime, image generation, and video generation are not claimed. Contract support does not establish release certification.
- vLLM support depends on the tasks served by the local OpenAI-compatible server. For Qwen2.5 deployments that use automatic tool choice, start vLLM with `--enable-auto-tool-choice --tool-call-parser hermes`; without both flags, tool definitions can be present while no tool call is parsed, and the agent smoke fails closed. Custom endpoints such as tokenize, rerank, classify, and score are outside the stable SDK contract.
