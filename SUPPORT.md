...
The catalog and offline tests cover GPT-6 Astra on OpenAI/Azure Responses, Claude Fable 5.1 and restricted-access Mythos 5.1, Gemini 3.8 Flash on Gemini/Vertex, and Qwen3.8-Max-0902 with its documented dated alias. Older IDs stay distinct. The September 5 DeepSeek vision preview has since been superseded by the September 16 V4.1 Flash routes documented below; Flash user-image support is Beta, while V4 Pro remains text-only. Files, Responses, and other DeepSeek endpoints remain outside this adapter's contract. See [the dated source review](./docs/MODEL_REFRESH_2026_09.md) for exact capabilities, limitations, prices, and unavailable evidence. Offline tests are not live certification.
