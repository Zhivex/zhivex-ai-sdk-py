...
The catalog adds GPT-6 Astra (OpenAI and Azure deployment reference), Claude Fable 5.1 and restricted Mythos 5.1, Gemini 3.8 Flash (Gemini and Vertex), Qwen3.8-Max-0902, and Experimental DeepSeek V4 Flash Vision. New model IDs retain provider-specific validation and do not imply live release certification. See [verified sources and limitations](./docs/MODEL_REFRESH_2026_09.md).
...
This matrix combines runtime support metadata with validated, versioned certification evidence via `scripts/generate_support_matrix.py`.
Regenerate the README and SUPPORT blocks with `python3 scripts/generate_support_matrix.py --write-docs`.
    ...
    gateway = create_gateway(
        GatewayConfig(
            ...
            model_costs_per_1k_tokens={
                ...
                "anthropic": {"claude-sonnet-5": 0.01},
            },
        )
    )
...
`create_cached_generate_middleware(...)` also coalesces concurrent requests that resolve to the same cache key: one downstream generation is shared by all waiters, while distinct keys continue independently. In-flight entries are process-local, are removed after success or failure, and are shielded so cancelling one waiter does not cancel shared provider work.

`create_file_generate_cache(...)` publishes each value through a unique temporary file in the configured directory, flushes and syncs the file, then uses same-filesystem `os.replace(...)`; readers therefore observe the previous complete JSON document or the new one. Corrupt, truncated, incompatible, symlinked, or non-regular entries fail closed with the top-level `ValidationError` instead of being treated as cache misses. A failed write removes its own temporary immediately. Each later write also scans at most 256 directory entries and removes at most 32 cache-owned temporaries older than 24 hours; fresh temporaries and unrelated files are retained.

Use the file cache only on an application-owned local filesystem whose same-directory `os.replace(...)` is atomic. Cache keys are SHA-256-derived and cannot select paths, and POSIX readers use `O_NOFOLLOW` when available, but the configured directory must not be writable by untrusted users. Linux is the CI-validated support target for this cache; macOS uses the same local-filesystem path, while Windows and network/distributed mounts require deployment-specific atomicity and crash-recovery validation.
...
It only runs providers that have the required credentials and model IDs configured, and you can scope it with `ZHIVEX_SMOKE_PROVIDERS=openai,anthropic,azure-openai,gemini,vertex,qwen,kimi,deepseek,meta,vllm`. Run `ZHIVEX_SMOKE_PROVIDERS=openai make smoke-agents` for the strict agent-first gate: every explicitly selected provider must execute its generation smoke and a real `run_agent(...)` loop that calls a local nonce-validation tool exactly once, consumes its result, and finishes successfully. Without a selector, strict mode keeps the local-development rule that at least one configured provider must execute. `ZHIVEX_SMOKE_PORTABLE_CERTIFICATION=1` executes streaming and structured output with synthetic markers; vLLM also executes portable retrieval, while Qwen, Kimi, and DeepSeek record retrieval as unsupported rather than successful. Secret values, authenticated URLs, paths, query strings, prompts, and model responses are excluded from retained evidence; protected certification logs use allowlisted diagnostic codes. PyPI and TestPyPI publication additionally require the protected `release-smoke` GitHub environment; its configured provider subset runs from the exact verified wheel before the Trusted Publisher job can start. The separate manual `Tier-1 provider certification` workflow covers both cohorts one target at a time from the exact published wheel: HU7 (OpenAI, Anthropic, Azure OpenAI, Gemini, Vertex) and HU8 (Qwen, Kimi, DeepSeek, Meta Standard, vLLM). Missing credentials or deployments are retained as explicit blockers and do not remove providers from Tier-1 or invalidate successful targets. The release policy pins `gpt-5.6-luna` plus Beta `muse-spark-1.2-contributor` for non-sensitive synthetic canaries; that evidence certifies only those exact targets and does not certify Meta Standard. Local HU8 exact-wheel results remain integration evidence until the protected workflow runs. Tier-1 setup details live in [docs/providers/tier-1.md](./docs/providers/tier-1.md), and the evidence contract lives in [docs/PROVIDER_CERTIFICATION.md](./docs/PROVIDER_CERTIFICATION.md). Optional Google media smoke checks are gated behind `ZHIVEX_SMOKE_GOOGLE_MEDIA=1` and model IDs such as `ZHIVEX_SMOKE_GEMINI_IMAGE_MODEL`, `ZHIVEX_SMOKE_GEMINI_VIDEO_MODEL`, `ZHIVEX_SMOKE_GEMINI_MEDIA_MODEL`, `ZHIVEX_SMOKE_VERTEX_IMAGE_MODEL`, `ZHIVEX_SMOKE_VERTEX_VIDEO_MODEL`, and `ZHIVEX_SMOKE_VERTEX_MEDIA_MODEL`. Ollama smoke runs default to `http://localhost:11434/v1` and can be redirected with `ZHIVEX_SMOKE_OLLAMA_BASE_URL`. Qwen smoke uses `DASHSCOPE_API_KEY` or `QWEN_API_KEY`, supports `ZHIVEX_SMOKE_QWEN_BASE_URL` and `ZHIVEX_SMOKE_QWEN_RESPONSES_BASE_URL` overrides, and can optionally validate embeddings, ASR, and TTS with `ZHIVEX_SMOKE_QWEN_EMBEDDING_MODEL`, `ZHIVEX_SMOKE_QWEN_ASR_MODEL` plus `ZHIVEX_SMOKE_QWEN_ASR_AUDIO_PATH`, and `ZHIVEX_SMOKE_QWEN_TTS_MODEL`. Kimi smoke uses `MOONSHOT_API_KEY` or `KIMI_API_KEY`, with optional `MOONSHOT_BASE_URL` or `ZHIVEX_SMOKE_KIMI_BASE_URL`. DeepSeek smoke uses `DEEPSEEK_API_KEY` and `ZHIVEX_SMOKE_DEEPSEEK_MODEL`, with optional `DEEPSEEK_BASE_URL` or `ZHIVEX_SMOKE_DEEPSEEK_BASE_URL`. Meta smoke uses `MODEL_API_KEY` and an explicit `ZHIVEX_SMOKE_META_MODEL`; use `muse-spark-1.2` Standard for sensitive smoke data and do not infer formal certification until that exact protected run is recorded.
