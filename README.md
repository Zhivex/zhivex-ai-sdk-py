...
- Anthropic `stop_reason="refusal"` is normalized as `finish_reason="refusal"` while preserving `provider_finish_reason`. Gateway routes use `fallback_on_refusal=True` by default so a refusal from the primary target can move to configured fallbacks.
