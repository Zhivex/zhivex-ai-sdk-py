...
DeepSeek is tier-1 for portable text generation, streaming, JSON structured output, callable tools, and reasoning through its Chat Completions API. The stable factory targets `deepseek-flash` (V4.1) and the separate `deepseek-v4-pro` text contract, preserves reasoning state across tool loops, and rejects retired model IDs or incompatible thinking options before dispatch. Strict-tool and prefix beta routing plus raw `provider_options` remain provider-specific beta/experimental behavior; Flash V4.1 user-image vision remains a Beta SDK extension; files, embeddings, audio, moderation, and hosted tools are not claimed.
...
DeepSeek now serves `deepseek-v4-flash` and `deepseek-v4-flash-vision-exp` with V4.1 Flash. These exact IDs remain accepted and separately cataloged as deprecated routes with replacement `deepseek-flash`. Their user-image path remains Beta in the SDK; this upstream change does not promote vision into the Stable Tier-1 cohort or add Files/Responses endpoints.
